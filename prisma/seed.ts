...
// For seeding, we can use the standard PrismaClient
// The database URL is loaded from environment variables via prisma.config.ts
const prisma = new PrismaClient();

async function main() {
  console.log('🌱 Starting database seed...');

  // Seed projects from your existing config
  const projects = [
    {
      title: "AMP Vending",
  ...
  for (const project of projects) {
    // Use upsert with a unique combination to avoid duplicates
